const SkeletonItem = () => {
  return (
    <div className="animate-pulse grid md:flex justify-between items-center gap-4 w-full p-5 border-dashed border-b border-slate-200 dark:bg-slate-900 dark:border-slate-700 last:border-none">
      <div className="h-5 bg-gray-200 dark:bg-slate-900 rounded w-24"></div>
      <div className="w-auto gap-3 flex justify-start md:justify-center items-center">
...
const SkeletonTotalTransactions = () => {
  return (
    <div className="animate-pulse grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-6 w-full">
      {Array.from({ length: 2 }).map((_, index) => {
        return (
          <div key={index} className="h-24 md:h-32 border border-gray-200 dark:bg-slate-900 rounded-lg w-full p-6 md:p-8 grid gap-1">
